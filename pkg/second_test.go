...
func convertToSets(str string) []map[string]int {
	str = strings.TrimSpace(str)
	setsData := strings.Split(str, ";")
	...
	for _, set := range setsData {
		...
		for _, item := range strings.Split(set, ",") {
			...
			countColor, _ := strconv.Atoi(count)
...
func TestIsGamePossible(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			maxCubes:   map[string]int{"red": 12, "blue": 14, "green": 13},
			wantResult: true,
		},
		{
			name:       "Impossible Game",
			sets:       "3 blue, 13 red; 1 red, 2 green, 6 blue; 2 green",
			maxCubes:   map[string]int{"red": 12, "blue": 14, "green": 13},
			wantResult: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sets := convertToSets(tt.sets)
			if gotResult := isGamePossible(sets, tt.maxCubes); gotResult != tt.wantResult {
				t.Errorf("isGamePossible() = %v, want %v", gotResult, tt.wantResult)
			}
		})
	}
}
...
Game 100: 16 blue, 12 red, 3 green; 2 green, 7 blue; 5 blue, 4 green; 10 blue, 6 red, 6 green; 5 red, 12 blue, 2 green; 9 red, 12 blue, 11 green
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotResult := processGames(tt.inputData); gotResult != tt.wantResult {
				t.Errorf("processGames() = %v, want %v", gotResult, tt.wantResult)
			}
		})
	}
}
...
func BenchmarkProcessGames(b *testing.B) {
	currOutput := log.Writer()
	log.SetOutput(io.Discard)

	for i := 0; i < b.N; i++ {
		processGames(benchmarkInput)
	}
